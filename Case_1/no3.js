const fruits = [
  ...
  function separateFruitsByType(fruits) {
    ...
    for (const fruit of fruits) {
      ...
      if (!fruitContainers[type]) {
        fruitContainers[type] = { fruits: [], totalStock: 0 };
      }
      fruitContainers[type].fruits.push(fruit.fruitName);
      fruitContainers[type].totalStock += fruit.stock;
    }
  ...
  for (const type in separatedFruits) {
    console.log(`Wadah untuk buah tipe ${type}: ${separatedFruits[type].fruits.join(", ")}`);
    console.log(`Total stok di wadah: ${separatedFruits[type].totalStock}`);
  }
